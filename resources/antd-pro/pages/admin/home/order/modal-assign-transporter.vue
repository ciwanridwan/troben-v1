<template>
  <a-modal :visible="visible" @cancel="onCancel" :width="840" centered :closable="true" :mask-closable="true" :ok-button-props="{ props: {disabled: true} }">
    <template slot="title">
      ...
      <a-col :span="14" class="trawl-border-right" style="padding-right:24px">
        <a-space direction="vertical" :size="24">
          ...
          <a-row type="flex">
            ...
            <a-col :span="leftSize">
              <address-component>
                ...
                <h5 class="trawl-text-normal"  slot="phone">{{order.sender_phone}}</h5>
                <p slot="address">
          ...
          <a-row type="flex">
            ...
            <a-col :span="leftSize">
              <address-component :receiver="true">
                ...
                <h5 class="trawl-text-normal"  slot="phone">{{order.receiver_phone}}</h5>
                <p slot="address">
          ...
          <a-row>
            ...
            <a-col :span="leftSize">
              <h5 class="trawl-text-normal">Jumlah Kg: <b>{{totalItemsWeight}} Kg</b></h5>
            </a-col>
          </a-row>
        </a-space>
      </a-col>
      <a-col :span="10">
        <a-layout>
          <a-layout-content class="trawl-bg-white">
            ...
            <a-space direction="vertical">
              ...
              <a-radio-group style="padding:0px 24px" v-model="chosenPartner">
                <a-space direction="vertical">
                  <a-card v-for="(partner, index) in items.data" :key="`partner-`+index">
                    <a-space direction="vertical">
                      <a-row type="flex">
                        <a-col :span="20">
                          <a-space direction="vertical">
          ...
          <a-layout-footer class="trawl-bg-white" style="padding:24px">
            <a-button @click="onOk" class="trawl-button-success" block :disabled="! isReadyToSubmit" :loading="loading">Tugaskan</a-button>
          </a-layout-footer>
        </a-layout>
      </a-col>
    </a-row>
...
export default {
  ...
  computed: {
    ...
    totalItemsWeight() {
      return this.order?.items?.reduce((borrow, item) => {
  ...
  methods: {
    ...
    getPartners: _.debounce(function () {
      this.loading = true
      this.$http.get(this.routeUri('admin.master.partner'), {
        params: {
          per_page: 2,
          q: this.search
        }
      }).then(({ data: responseData }) => this.items = responseData)
    ...
    async save() {
      ...
      try {
        const response = await this.$http.patch(this.routeUri('admin.home.assign', {
