<template>
  <a-card :loading="loading">
    <a-table :columns="customerColumns" :data-source="items.data" :pagination="trawlbensPagination" @change="handleTableChanged">
...
import TrawlCard from "../../components/trawl-card";
import TrawlTable from "../../components/trawl-table";
...
export default {
  name: "customer-list",
  components: {DetailButton, DeleteButton, EditButton, TrawlTable, TrawlCard},
  created() {
  ...
  methods: {
    getItems() {
      this.loading = true
      this.$http.get(this.routeUri('admin.master.customer'), { params: this.filter })
