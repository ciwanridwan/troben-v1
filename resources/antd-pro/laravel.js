...
const laravel = {
  computed: {
    config() {
      if (localStorage.getItem('antd_config') !== null)
        Object.assign(config.layout, JSON.parse(localStorage.getItem('antd_config')))
  ...
  methods: {
    ...
    pushState(state) {
      history.pushState(state, document.title, location.origin + location.pathname + "?" + this.$qs.stringify(state, {encode: false}))
    },
    isAuthenticated() {
    ...
    initial(v) {
      return v.match(/(^\S\S?|\b\S)?/g).join("").match(/(^\S|\S$)?/g).join("").toUpperCase()
    },
    routes() {
    ...
    routeUri(name, replacer = {}, isPath = false) {
      let path = isPath ? name : '/' + (this.route(name) ? this.route(name).uri : window.location.pathname)
      return path.replace(/{([a-z0-9_]+)}/gi, (match, key) => replacer[key])
    },
    copy(value, message = null) {
      this.$clipboard(value)

      this.$message.success((message === null) ? '"' + value + '" has been copied to your clipboard.' : message)
    },
    currency(number) {
    ...
    handleXhrError(error) {
      this.$notification.error({
        message: (error.response.data.code) ? error.response.data.code : "UNKNOWN",
