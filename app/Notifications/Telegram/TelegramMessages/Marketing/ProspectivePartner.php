...
class ProspectivePartner extends Notification
{
    ...
    public function toTelegram($notifiable)
    {
        ...
        if ($notifiable['type'] === Partner::TYPE_BUSINESS) {
            $message .= "'Mitra Bisnis'";
        } elseif ($notifiable['type'] === Partner::TYPE_POOL) {
            $message .= "'Mitra Pool Warehouse'";
        } elseif ($notifiable['type'] === Partner::TYPE_SPACE) {
            $message .= "'Mitra Space'";
        } elseif ($notifiable['type'] === Partner::TYPE_TRANSPORTER) {
            $message .= "'Mitra Transporter Mobil'";
        } else {
            $message .= "'Mitra Kurir Motor'";
        }
        $message .= "*\n";

        $message .= 'Nama: *'.$notifiable['nama_depan'].' '.$notifiable['nama_belakang']."*\n".
            'No. HP: *'.$notifiable['handphone'];
        if (! empty($notifiable['handphone1'])) {
            $message .= ' / '.$notifiable['handphone1'];
        }
        ...
        if (in_array($notifiable['type'], [Partner::TYPE_BUSINESS,Partner::TYPE_SPACE,Partner::TYPE_POOL])) {
            $message .= 'Alamat Usaha: *'.$notifiable['alamat_usaha']."*\n".
        ...
        if (! in_array($notifiable['type'], Partner::getAvailableTypes())) {
            $message .= 'Merek Kendaraan: *'.$notifiable['merek']."*\n".
            'Tipe Kendaraan: *'.$notifiable['tipe']."*\n".
            'Tahun Kendaraan: *'.$notifiable['tahun']."*\n";
        }

        if ($notifiable['type'] === Partner::TYPE_BUSINESS) {
            $url = 'https://crm.trawlbens.id/administrator/mitra_business';
        } elseif ($notifiable['type'] === Partner::TYPE_POOL) {
            $url = 'https://crm.trawlbens.id/administrator/mitra_pool_warehouse';
        } elseif ($notifiable['type'] === Partner::TYPE_SPACE) {
            $url = 'https://crm.trawlbens.id/administrator/mitra_space';
        } elseif ($notifiable['type'] === Partner::TYPE_TRANSPORTER) {
            $url = 'https://crm.trawlbens.id/administrator/mitra_transporter_mobil';
        } else {
            $url = 'https://crm.trawlbens.id/administrator/mitra_kurir_motor';
        }
